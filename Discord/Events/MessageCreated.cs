            ...
            if (arg.Message.Content.StartsWith("/"))
            {
                ...
                switch (cmdSplit[0])
                {
                    case "register":
                        ...
                        {
                            if(Utils.UserTools.UserExists(plchd))
                            {
                                arg.Message.Channel.SendMessageAsync("Die BenutzerID " + plchd + " wurde bereits registrieret.");
                                break;
                            }
                        ...
                        {
                            arg.Message.Channel.SendMessageAsync("Bitte prüfe deine angegeben Daten, es scheint als wären Sie nicht in Ordnung gewesen.");
                            arg.Message.Channel.SendMessageAsync("Nutze /register (Nummer) (Password) oder /stop (Nummer) | Urlaub: W.I.P");
                        }
                        break;
